#'@export
confint.poisreg <- function(object, parm = NULL, level = 0.95, ...) {
  V <- vcov(object)
...
#'@export
confint.zapoisreg <- function(object, parm = NULL, level = 0.95, ...) {
  V <- vcov(object)
  par.hat <- object$fit$par
  alpha <- 1 - level
  d <- stats::qnorm(1 - alpha / 2) * sqrt(diag(V))
...
#'@export
confint.zipoisreg <- function(object, parm = NULL, level = 0.95, ...) {
  p <- object$p
  q <- object$q
  V <- vcov(object)
  par.hat <- object$fit$par
