#'@export
print.summary.zapoisreg <- function(x, ...) {
  if (x$approach == "mle") {
    cat("Call:\n")
    print(x$call)
    cat("\n")
...
#'@export
summary.zapoisreg <- function(object, ...) {
  if (object$approach == "mle") {
    p <- object$p
    labels <- c("psi", object$labels)
    ...
    s <- rstan::summary(object$fit, pars = c("psi", "beta"))
