...
print.summary.cmzinb <- function(x, ...) {
  if (x$approach == "mle") {
    ...
    cat("\n")
    cat("--- \n")
    cat("Theta = ", x$theta, " ", "Std. = ", x$std_theta)
  } else {
    cat("Call:\n")
    print(x$call)
    cat("\n")
...
summary.cmzinb <- function(object, ...) {
  ...
  if (object$approach == "mle") {
    ...
    coefficients <- object$fit$par[2:k]
    V <- vcov(object)
    vt_log <- 1 / object$fit$hessian[k, k]
    vt <- ((object$fit$par[1])^2) * vt_log
    ...
    res <- list(
      ...
      thin = object$fit@sim$thin,
      iter = object$fit@sim$iter,
      approach = object$approach
    )
  }

  class(res) <- "summary.cmzinb"
